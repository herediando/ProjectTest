  ...
  global $mensaje;
    ...
    $query = "select * from relhabitantes 
           ...
           if (!$resultado)
             {
         echo mysql_error();
               echo "No se ejecuto la Consulta para Eliminar de la BD: " . mysql_error();
               exit;
              }
           $mensaje = "Datos a Eliminar Consultados Exitosamente";
}
...
$jubilado        = ""; $cne             = ""; $status        = ""; $fechastatus =  "";
    
      $cedula=0;
      $tipo  ="";     
    $mensaje = "Error Habitante a ELIMINAR NO existe, Por Favor Verificar DATOS, Cedula: ".$_POST['cedula'];
       ...
       if(mysql_num_rows($resultado)==0)
          {
       echo "<script> alert('Cedula Habitante a Eliminar NO existe, Por Favor Verificar'); history.go(-1)</script>";
       die;
       }   
      ...
      echo "No se pudo ejecutar con exito la CONSULTA en la BD: " . mysql_error();
          exit; 
      echo "<script> alert('Consulta Habitante a Eliminar Sin Exito'); history.go(-1)</script>";  die;
          }
     else
     { 
     ...
     if(isset($_POST['cedula']) && !empty($_POST['cedula']))
      {
       ...
       $query = "select * from padres where (padcedulahijo = '$_POST[cedula]')";
       ...
       while ($fila = mysql_fetch_array($resultado))
               {
               $cedulapadre = $fila['padcedulapadre'];       
                 $cedulamadre = $fila['padcedulamadre']; 
           }
      }
   }  
...
<form action="BDEliminarHabitantes.php" method="post" name="form1" >

<fieldset>
<legend><center><h2>Eliminar Registro de Habitantes</h2></center></legend>
...
<td><b>Primer Apellido</b> <font color="#FF0000">*</font>
...
<br>
<b>
<center><input type="submit" name="eliminacion" value="Eliminar"></center>  
</b>
</fieldset>
</form>
</table> 
